...
GPU), pulling the whole pipeline $\mixedAnnTotalSpeedup\times$ faster.
...
\texttt{scale}, \texttt{pca}, \texttt{neighbors}, \texttt{leiden},
\texttt{louvain}, \texttt{umap}). The gaps are informative rather than
fatal: \texttt{highly\_variable\_genes} fails \emph{only} when called
standalone (the fused HVG inside \texttt{ov.pp.preprocess} works, which
is the canonical entry point); \texttt{filter\_cells} /
\texttt{filter\_genes}, \texttt{normalize\_pearson\_residuals} and
\texttt{score\_genes\_cell\_cycle} are not yet wired through the
\texttt{is\_oom} dispatch and raise a clear error rather than silently
mis-computing; and \texttt{anndata\_to\_GPU} / \texttt{anndata\_to\_CPU}
require the optional \texttt{rapids\_singlecell} dependency (an
environment matter, not an \texttt{anndataoom} limitation). None of the
failures corrupt data --- they surface as exceptions at the call site.
