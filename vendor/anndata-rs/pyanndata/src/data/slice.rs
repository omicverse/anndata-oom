...
pub fn to_select_info(ob: &Bound<'_, PyAny>, shape: &Shape) -> PyResult<SelectInfo> {
    let ndim = shape.ndim();
    if is_none_slice(ob)? {
        Ok(std::iter::repeat_with(SelectInfoElem::full).take(ndim).collect())
    } else if ob.is_instance_of::<pyo3::types::PyTuple>() {
        ob.try_iter()?.zip(shape.as_ref())
            .map(|(x, len)| to_select_elem(&x?, *len))
            .collect()
    } else {
        // Single-dim indexing (slice / int / bool mask / int list / ndarray):
        // select along the first axis, leave remaining axes as full selects.
        // Mirrors numpy's `arr[x]` == `arr[x, :, :, ...]` semantics.
        let dims = shape.as_ref();
        let first = to_select_elem(ob, dims[0])?;
        let rest = std::iter::repeat_with(SelectInfoElem::full).take(ndim - 1);
